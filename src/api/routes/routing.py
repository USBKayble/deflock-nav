...
def point_to_segment_distance(point, line_start, line_end, dist12: float = None) -> float:
    ...
    if dist12 == 0:
        return dist13
    if dist13**2 > dist12**2 + dist23**2:
        return dist23
    if dist23**2 > dist12**2 + dist13**2:
        return dist13
...
def in_field_of_view(
    heading: float, camera_direction: float, fov_angle: float = 120
) -> bool:
...
async def calculate_route(request: RouteRequest):
    # Get OSRM Routes
    coords_str = (
        f"{request.start.lon},{request.start.lat};{request.end.lon},{request.end.lat}"
    )
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if data.get("code") != "Ok":
                raise HTTPException(
                    status_code=400, detail=f"OSRM Error: {data.get('code')}"
                )
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            cameras = await get_cameras(
                bbox=bbox, type=None, operator=None, limit=500, db=db
            )
